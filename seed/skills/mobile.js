...
export default function buildMobileSkills() {
  ...
  const rn = mk('React Native', 'mobile', null, {
    ...
    gotchas:
      'Flexbox defaults differ from web: flexDirection defaults to "column" (not row), and flex: 1 requires parent to have a defined height or flex itself.\nInline arrow functions in renderItem create a new function every render; combine stable callbacks with memoized rows when list re-render cost is measurable.\nAnimated runs on the JS thread unless you use the native driver for supported properties. Use Reanimated worklets for complex gesture-driven animations.\nStyleSheet.create does not provide runtime validation for every style value — a typo in a colour string can still fail at runtime.\nAndroid back button must be handled explicitly with BackHandler or navigation.goBack() — unhandled presses close the app.\nShadow props differ between iOS (shadowColor, shadowRadius etc.) and Android (elevation) — use a utility wrapper.\nNew Architecture (Fabric/TurboModules) changes native module authoring significantly — mixing old and new bridge assumptions causes hard-to-debug issues.',
    flashcards: [
      card('How does React Native render to native UI without a WebView?', 'RN runs JavaScript in a JS engine (Hermes by default in modern RN) and uses the RN renderer to create native host views such as UIView on iOS and Android View on Android. There is no DOM or browser WebView; JS and native communicate through the legacy bridge or, in the New Architecture, through JSI-backed native interfaces.'),
      card('What is JSI and why does it replace the async bridge?', 'JSI (JavaScript Interface) lets the JS engine call C++ host objects/functions directly, avoiding the legacy bridge\'s async batched serialization path. It enables synchronous native APIs where designed and reduces bridge overhead; it is not a magic guarantee that every call is zero-cost or zero-copy.'),
      card('What is the difference between Fabric and the old renderer?', 'Fabric is the New Architecture renderer implemented in C++ with better scheduling, synchronous layout access, and support for React 18 concurrent rendering. The old renderer used the legacy shadow tree and bridge-driven mounting pipeline, so updates were harder to prioritize or interrupt.'),
      ...
      card('Why must StyleSheet.create be called outside render?', 'In modern RN, StyleSheet.create mostly returns the same style object and freezes each style in development for validation. Keeping it outside render still matters because it avoids recreating objects every render, preserves referential equality for memoized children, and keeps style definitions readable.'),
      ...
      card('What is the difference between useNativeDriver: true and false in Animated?', 'useNativeDriver: true serialises a supported animation graph to native once, so JS is not involved on every frame; it works mainly for transform and opacity. false drives updates from the JS thread, so heavy JS work can cause jank. Native-driven animations can still drop frames if the native UI thread is overloaded.'),
      card('How does Reanimated 3 differ from the Animated API?', 'Reanimated worklets run in a separate UI-thread runtime via JSI rather than on the React JS thread. Shared values can update animated styles without React re-rendering or per-frame bridge traffic, which makes complex gesture/animation work much more reliable under JS-thread load.'),
      card('What is the Metro bundler and what is its role?', 'Metro is RN\'s JavaScript bundler — it resolves modules, transforms JSX/TS, and serves the bundle to the device during development. In production, it emits the JS bundle and assets that are embedded in the native app, or delivered by a configured OTA update system.'),
      card('What is CodePush and when should you not use it?', 'CodePush delivers JavaScript bundle and asset updates OTA, but Microsoft retired App Center and moved CodePush to a standalone/self-hosted path. Do not use OTA updates for native-code changes, new native modules, permission/entitlement changes, or SDK upgrades — those require a full app-store release.'),
    ],
    apis: [
      api('StyleSheet.create', 'StyleSheet.create(styles: { [key: string]: ViewStyle | TextStyle | ImageStyle })', 'Creates a named style object; in modern RN it is mostly an identity helper with development-time freezing and useful static typing/IDE checks.', 'plain style object map', 'style object map', "const styles = StyleSheet.create({\n  container: { flex: 1, backgroundColor: '#fff' },\n  title: { fontSize: 18, fontWeight: '700' },\n});\nStyleSheet.hairlineWidth // thinnest native line", 'Call at module level, not inside render, so references stay stable. Does not validate every runtime style value.'),
  ...
  skills.push(mk('Core Components', 'mobile', rn.id, {
    ...
    flashcards: [
      card('Why use SafeAreaView instead of a plain View at the screen root?', 'A safe-area wrapper prevents content from sitting under notches, rounded corners, status bars, and home indicators. RN core SafeAreaView is iOS-only and deprecated; production apps usually use react-native-safe-area-context, which supports iOS and Android insets.'),
  ...
  skills.push(mk('Styling', 'mobile', rn.id, {
    definition:
      'React Native uses a JavaScript-based styling system inspired by CSS but not identical to it. StyleSheet.create is a style-definition helper that keeps style objects named, stable, and development-validated. Flexbox is always active but defaults differ: flexDirection is "column", alignItems is "stretch". There is no cascade, no inheritance beyond Text inside Text, and no CSS units — all dimensions are density-independent pixels.',
    codeExample:
  ...
  skills.push(mk('Lists & Performance', 'mobile', rn.id, {
    ...
    flashcards: [
      ...
      card('Why should renderItem often be useCallback-wrapped?', 'FlatList is a PureComponent, so stable props help it avoid unnecessary work. An inline renderItem creates a new function every parent render; with memoized row components and stable item props, useCallback helps visible rows avoid pointless re-renders. It is a performance tool, not a correctness requirement.'),
    ],
  }));
  ...
  skills.push(mk('Networking', 'mobile', rn.id, {
    ...
    flashcards: [
      ...
      card('Why is AbortController important for RN networking?', 'Users navigate away from screens mid-request. AbortController cancels fetch work you no longer need, avoids wasted network/CPU, and prevents stale responses from updating newer UI state. Modern React ignores many unmounted setState cases, but cancellation is still the cleaner lifecycle pattern.'),
  ...
  skills.push(mk('Storage', 'mobile', rn.id, {
    ...
    flashcards: [
      ...
      card('What should never be stored in AsyncStorage or plain MMKV?', 'Auth tokens, refresh tokens, private keys, and sensitive PII should not go in unencrypted storage. Use Keychain/Keystore-backed storage for secrets; note that iOS Keychain items can survive app reinstall unless configured/cleaned up, while Android app data is normally removed on uninstall.'),
  ...
  skills.push(mk('Native Modules & New Architecture', 'mobile', rn.id, {
    ...
    flashcards: [
      ...
      card('What is Fabric?', 'Fabric is the New Architecture\'s C++ renderer. It supports React 18 concurrent rendering, uses a more consistent cross-platform shadow tree, and enables synchronous layout/native access patterns that were impractical through the legacy bridge. Reanimated and Gesture Handler benefit from JSI/UI-runtime integration alongside this architecture.'),
  ...
  skills.push(mk('Build, Deploy & Debugging', 'mobile', rn.id, {
    definition:
      'Production RN apps require Android keystore signing, iOS provisioning profiles, R8/ProGuard for code shrinking, and a CI pipeline. EAS Build abstracts much of the native build environment. Modern debugging should start with React Native DevTools, native logs via adb/Xcode, and platform profilers; Flipper is legacy/deprecated in newer React Native versions.',
    codeExample:
    ...
    flashcards: [
      ...
      card('What is Flipper and should new React Native apps rely on it?', 'Flipper is a desktop debugging platform that older RN apps used for network, layout, storage, and custom plugin inspection. New RN projects should prefer React Native DevTools and native platform tools because RN has moved away from Flipper as the default debugger.'),
    ],
  }));
  ...
  skills.push(mk('App Lifecycle & Permissions', 'mobile', rn.id, {
    ...
    flashcards: [
      card('What AppState values should you handle?', '"active" means foreground and interactive. "background" means the app is not visible. "inactive" is an iOS transitional/interruption state. RN types also include "unknown" for initial/native uncertainty and "extension" for iOS app-extension contexts, so production code should handle unexpected states defensively.'),
  ...
  skills.push(mk('New Architecture (Fabric / TurboModules / Hermes)', 'mobile', rn.id, {
    ...
    flashcards: [
      card('What is the primary advantage of TurboModules over legacy NativeModules?', 'TurboModules are codegen-typed, JSI-backed modules that can be loaded lazily and can expose synchronous methods when the spec allows it. Legacy NativeModules were eagerly registered through the bridge and primarily communicated through asynchronous serialized messages.'),
      card('What does the New Architecture enable that was impractical with the old bridge?', 'It enables JSI-backed native access, optional synchronous native methods, Fabric concurrent rendering, direct layout/native reads without bridge round-trips, and better UI-thread integrations for gestures and animations. Some patterns had workarounds before, but the New Architecture makes them first-class and far cheaper.'),
  ...
  const subRnPerf = mk('RN Performance (Deep Dive)', 'mobile', rn.id, {
    definition:
      "React Native's rendering pipeline keeps JavaScript on a dedicated thread, and heavy synchronous JS work can miss the 16.67ms frame budget needed for 60fps interactions. State updates flow through React reconciliation, shadow tree/layout work, and native mounting via the legacy bridge or Fabric/JSI in the New Architecture. Performance wins come from reducing per-frame JS work, virtualizing long lists, memoizing referentially-stable props where it matters, deferring non-critical computation, and keeping native updates efficient.",
    codeExample:
    ...
    gotchas:
      "React.memo is ineffective when the parent passes inline objects, arrays, or arrow functions as props because shallow comparison sees new references every render.\nuseCallback on renderItem helps most when row components are memoized and item props are stable; measure before adding memoization everywhere.\ngetItemLayout height must exactly match the StyleSheet value — an off-by-one causes scrollToIndex to land on the wrong item.\nremoveClippedSubviews can cause blank flashes on iOS during fast scrolling — useful on Android, test before enabling globally.\nContext with high-churn values (scroll position, animation progress) cascades re-renders to all consumers — use Reanimated shared values or selector-based external stores for high-frequency state instead.",
    flashcards: [
      ...
      card('RN rendering pipeline — what happens on a state update?', 'JS runs React render/reconciliation, RN updates the shadow tree, Yoga computes layout, then native mounting applies view mutations on the UI thread. Old architecture sends batched serialized work over the bridge; New Architecture uses Fabric/JSI with a C++ renderer and lower-overhead native access. Bottlenecks usually live in JS render work, layout, or excessive native updates.'),
      ...
      card('FlatList vs ScrollView for long lists?', 'FlatList virtualizes — it keeps only a render window of items mounted around the viewport. ScrollView mounts every child immediately, so memory and first-paint cost scale with list length. Use ScrollView for small static content; switch to FlatList when the list can grow or cells are non-trivial.'),
      ...
      card('How to detect memory leaks in production?', 'Look for memory that keeps increasing across a session, OOM crashes, or heap growth after repeating the same navigation flow. Use Xcode Instruments, Android Studio Profiler, Hermes profiling, and production crash/memory telemetry.'),
      card('Tools to profile a RN app?', 'React Native DevTools and React DevTools Profiler for component behavior, Hermes Profiler for JS CPU sampling, Xcode Instruments for iOS native performance, Android Studio Profiler for Android native performance, and Perf Monitor for quick frame-rate checks.'),
      ...
      card('Why do navigation stacks affect perf?', 'React Navigation commonly keeps previous screens mounted so back navigation is fast and state is preserved. Deep stacks can leave expensive components, subscriptions, and caches alive. Use lazy screens, detach inactive native screens where supported, pause work on blur, and unmount heavy screens only when losing state is acceptable.'),
  ...
  const subRnSec = mk('RN Security', 'mobile', rn.id, {
    ...
    gotchas:
      "react-native-keychain can fall back to less secure storage on emulators or older Android devices without hardware-backed key support — test on real hardware for production security validation.\nSSL pinning breaks when you rotate server certificates without backup pins or a migration path; pre-test the re-pin flow before it reaches production users.\nRoot/jailbreak detection is bypassable with tools such as Magisk and Frida — use it as one risk signal, not the only protection.\nconsole.log outside `if (__DEV__)` ships to production and is readable via `adb logcat` — strip logs in release builds using babel-plugin-transform-remove-console.\nOTA updates (CodePush/self-hosted OTA/Expo Updates) can bypass normal store review flows — verify bundle signatures client-side and stage rollouts carefully; a bad push can silently corrupt active sessions.",
    flashcards: [
      ...
      card('What is SSL pinning?', 'Client embeds or fetches an expected certificate/public-key hash and rejects TLS connections that do not match. It reduces MITM risk from rogue CAs or user-installed root certs, but it must be planned with certificate rotation and backup pins. RN apps usually implement it through native networking or a maintained pinning library.'),
      ...
      card('OTA update security risks?', "If the update channel is not signed and verified, an attacker or compromised service can deliver a malicious JS bundle. Use signed updates, restrict update sources, stage rollouts, and audit diffs before release. Treat CodePush/self-hosted OTA and Expo Updates as release infrastructure, not a casual CDN."),
...
Dead-lettered mutations silently dropped — always surface them to the user ("3 changes failed to sync") so they can resolve manually.`,
    flashcards: [
      card('What is offline-first architecture?', 'The app treats local storage as the immediate source of truth for UI: writes land locally first, reads come from the local store, and sync reconciles with the server in the background. The server is still usually the authority for cross-device/global truth. The app remains useful without network instead of treating offline as a broken state.'),
...
Scaling WS beyond one server requires a message bus (Redis pub/sub, Kafka) — otherwise server A cannot reach clients on server B.`,
    flashcards: [
      ...
      card('Reconnection strategy for WebSocket?', 'Use capped exponential backoff with jitter, for example min(base * 2^attempt, maxDelay) plus randomness, to prevent thundering herd after server restart. Replay missed events with sequence numbers or last-event-id when supported; otherwise reconcile local state from a server snapshot after reconnect.'),
...
Awareness state is ephemeral and lossy by design — never store business-critical data in awareness; it clears on disconnect.`,
    flashcards: [
      card('What is a CRDT?', 'Conflict-free Replicated Data Type. A data structure that can be updated independently on multiple replicas and merged deterministically when its rules are followed. Concurrent operations converge without a central conflict resolver, though access control, validation, and persistence still need server/application design.'),
      card('Yjs vs Automerge — which to pick?', "Yjs is widely used and very fast for collaborative text/lists with compact binary updates. Automerge is document/JSON-shaped and has improved a lot in v2, but usually has different trade-offs around data model and storage. Default to Yjs for editor-like collaboration; choose Automerge when its document model fits better."),
      card('Why is "awareness" separate from persistent state in CRDTs?', 'Awareness = ephemeral state such as cursor positions, typing indicators, and presence. It does not need durable merge semantics, so collaboration systems broadcast it separately instead of writing it into the document CRDT. Storing awareness in the CRDT would bloat history with session-only data.'),
...
Forgetting to handle duplicates — event processors must be idempotent; events can be delivered more than once in failure scenarios.`,
    flashcards: [
      ...
      card('Why is event schema evolution hard?', 'Events are immutable records of facts, so old event shapes remain in the log while code and product requirements evolve. Renames, splits, and semantic changes must be handled by versioned events, upcasters, compatibility readers, or additive-only discipline. You cannot assume a database migration will rewrite history safely.'),
...
Model update without an app release requires OTA delivery pipeline — plan for secure model signing and version management.`,
    flashcards: [
      ...
      card('What is quantization and why is it essential on mobile?', 'Quantization reduces model weights and sometimes activations from 32-bit floats to lower precision such as 16-bit, 8-bit, or 4-bit formats. It cuts model size, memory, and often latency, with a task-specific quality trade-off that must be measured. It is usually required for large mobile ML models.'),
      card('Can you run an LLM on-device?', "Yes, but constrained. Quantized small models can run on modern high-end phones with enough RAM and acceleration, using runtimes such as MLC LLM, llama.cpp-based bridges, ExecuTorch, or platform ML APIs. Expect slower throughput, thermal/battery limits, and smaller context windows than cloud models. Best for privacy-critical or offline tasks."),
...
Most apps don't need MFE — the overhead only pays off at 5+ teams with genuinely independent release cadences.`,
    flashcards: [
      ...
      card('App store policy concerns with MFE?', 'Apple allows interpreted code updates only within policy limits: they must not change the app\'s primary purpose or bypass review for major new functionality. Runtime-loaded MFEs are higher risk than pre-bundled modules, so keep remote bundles to reviewed/known capabilities, sign them, and consult policy before shipping feature changes this way.'),
      card('Cold start cost of MFE?', 'Loading a remote MFE bundle adds network + parse + execute time before the screen can render, often noticeable on the first visit. Solutions: prefetch likely MFEs, cache bundles, ship critical flows in the main bundle, and show resilient loading/error states. Measure on low-end devices and slow networks.'),
...
messaging().onMessage(async (remoteMessage) => {
  await syncFromPush(remoteMessage.data);
    ...
    gotchas: `Push delivery is best-effort — FCM/APNs can delay, collapse, throttle, or drop pushes. Always reconcile state on app foreground; never rely solely on pushes for critical data delivery.
iOS grants only a short background execution window after a background push and may skip delivery; heavy sync must be chunked or deferred to BGTaskScheduler when conditions allow.
Android Doze batches background work to maintenance windows — timing is non-deterministic and grows more restrictive over idle time; design sync to be eventual, not time-sensitive.
...
Battery vs freshness trade-off must be surfaced to users — allow sync frequency control in settings and throttle on low battery.`,
    flashcards: [
      card('What is push-sync architecture?', 'Server sends pushes to trigger background or next-foreground sync. Silent/data-only pushes can wake the app under platform limits, the app applies or fetches changes, then returns to sleep. User often opens to fresher local data. It is usually more battery-efficient than polling, but delivery is best-effort.'),
      ...
      card('iOS background execution limits?', 'iOS grants only a short background execution window after a background push, commonly around 30 seconds, and may skip delivery based on power/network heuristics. Heavy sync must be quick, chunked, and reconciled on foreground. BGProcessingTask can allow longer work, but only when iOS decides conditions are suitable.'),
      card('Android Doze mode — what is it?', 'When a device is unplugged and idle, Android Doze defers network access, jobs, and alarms into maintenance windows to save battery. Timing is not fixed and becomes more restrictive over time. It affects background sync and sometimes push timing, so apps must reconcile on foreground.'),
      card('How to do reliable background sync on RN?', 'Use platform schedulers: WorkManager/JobScheduler-backed libraries on Android and BGTaskScheduler/background-fetch style APIs on iOS. These are opportunistic, not exact timers. Make sync idempotent, persist queues, handle missed runs, and always reconcile when the app returns to foreground.'),
      card('Why is push delivery best-effort, not guaranteed?', "FCM/APNs optimize for battery, network, priority, and platform policy, so pushes can be delayed, collapsed, throttled, or dropped when a device is offline too long or quotas are exceeded. Apps must handle missed pushes by syncing on foreground and reconciling state; never rely solely on pushes for critical data."),
...
Teams lacking expertise in one stack can't maintain hybrid effectively — hybrid requires senior engineers comfortable with both Swift/Kotlin and React Native.`,
    flashcards: [
      ...
      card("What's the bridging cost?", 'In the legacy architecture, native/RN calls cross an async serialized bridge, so chatty communication can hurt performance. In the New Architecture, JSI/TurboModules reduce that overhead and can support sync calls, but crossing runtime/thread boundaries still has cost. Batch work and design native APIs to be coarse-grained.'),
      ...
      card('Memory footprint of hybrid apps?', 'Usually larger than a pure native app because the native shell, JS engine, RN runtime, and RN view infrastructure coexist. Cold start can be slower if RN initializes eagerly. Mitigate by lazy-initializing RN when the first RN surface is needed and reusing one runtime across RN screens where possible.'),
...
Profiling is mandatory — assume nothing about perf. Use Hermes Profiler (JS) + Xcode Instruments / Android Studio Profiler (native GPU/CPU) before optimizing.`,
    flashcards: [
      ...
      card('Animated API vs Reanimated 3 — pick which?', 'Animated with `useNativeDriver: true` is fine for simple transform/opacity animations. Reanimated 3 runs worklets in a UI-thread runtime, integrates deeply with gestures, and handles complex interactive animations better. Use the simpler API for simple cases; reach for Reanimated when animation logic depends on gestures, shared values, or per-frame decisions.'),
      ...
      card('Why can iOS animation feel smoother than Android by default?', 'iOS Core Animation can offload many layer animations efficiently to the render server/GPU, while Android performance varies more by device, OEM skin, and UI-thread workload. Android can be just as smooth when work is kept off the critical path. Reanimated helps both platforms by moving interactive animation logic away from the React JS thread.'),
