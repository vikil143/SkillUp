...
export default function buildStateSkills() {
  ...
  const subSecurity = mk('Auth & Security Architecture', 'state', archSkill.id, {
    ...
    flashcards: [
      ...
      card('How to store JWT safely in browser?', "Prefer an httpOnly, Secure cookie because JS cannot read it directly, reducing token theft from XSS. It is not a complete XSS defense because injected scripts can still trigger actions as the user. Cookies also need CSRF protection."),
